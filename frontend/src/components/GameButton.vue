...
// Adapt this import to your real type
// import type { GameCommandType } from '../api/serverPushClient';

type GameCommandType = string; // placeholder – plug your union here
...
const props = withDefaults(
  defineProps<{
    ...
    /** Global “busy” (e.g. backend in-flight) */
    busy?: boolean;

    /** Additional manual disable flag */
    disabled?: boolean;

    /** Commands that are not allowed by policy */
    canExecute?: boolean;

    /** If true, busy === disable */
...
const ariaLabel = computed(() => props.tooltip || props.label || String(props.action));

const normalizedHotkey = computed(() =>
  (props.hotkey ?? '').trim().toLowerCase(),
);

const normalizedHotkeyDisplay = computed(() => props.hotkey?.toUpperCase() ?? '');
...
function onGlobalKeydown(e: KeyboardEvent) {
  ...
  if (e.key.toLowerCase() === normalizedHotkey.value) {
    // don't steal shortcuts when focus is in an input
    ...
    // simulate click for consistent behaviour
    btnRef.value?.click();
  }
}
