...
//   GetGameState,
//   CurrentAttacker,
// } from './attackerHandBar';

// export interface HandCardRendererAssets {
//   cardBaseUrl?: string;
//   backImg?: string;
// }

// export function createHandCardRendererBase(): HandRenderer & {
//   applyOverlapSpacing(rowEl: HTMLElement, handSize: number): void;
// } {
//   function calcHandSpacingDynamic(rowEl: HTMLElement, handSize: number): number {
//     const first = rowEl.querySelector<HTMLElement>('.hand-card');
//     if (!first) return -40;
...
//   function applyOverlapSpacing(rowEl: HTMLElement, handSize: number): void {
//     const overlap = calcHandSpacingDynamic(rowEl, handSize);
//     const cards = Array.from(rowEl.querySelectorAll<HTMLElement>('.hand-card'));
//     cards.forEach((el, i) => {
...
// export function createDefaultHandCardRenderer(
//   assets: HandCardRendererAssets = {},
// ): HandRenderer & {
//   createHandCardRow(
...
//   function resolveImage(card: HandCardLike, isLast: boolean): string {
//     if (isLast && (card?.imgFront || card?.img)) {
//       return card.imgFront || (card.img as string);
//     }
//     if (!isLast && (card?.imgBack || card?.img)) {
//       return card.imgBack || backImg;
//     }
//     const front = fileNameToUrl(card?.fileName);
//     return isLast ? (front ?? backImg) : backImg;
//   }

//   function handsOf(gs: any, pid: string): HandCardLike[] {
//     return (
//       gs?.cards?.hands?.[pid] ??
